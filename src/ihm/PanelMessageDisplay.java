...
public class PanelMessageDisplay extends JScrollPane {
	...
	// Constructor
	public PanelMessageDisplay(FrameInterface frameInterface) {
	...
	public void displayMessage(final FilDeDiscussion fdd) {
		mainPanel.removeAll();
		for (final Message msg : fdd.getConversation()) {
			JPanel temp = new JPanel();
			JTextArea tp2 = new JTextArea(msg.getMsg());
			temp.setLayout(new BorderLayout());
			temp.addMouseListener(new MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent evt) {
					Message temp = new Message("", TypeMessage.REQUETE_STATUS_MESSAGE);
					temp.setIdMsg(msg.getIdMsg());
					System.out.println(temp.getIdMsg());
					frameInterface.getTube().send(temp);
					while (frameAfficherUtilisateur.getListUserRef() == null);
					...
					frameAfficherUtilisateur.setListUserRef(null);
				}
			});
			switch (msg.getType()) {
			case RECEIVED:
	...
	public FrameAfficherUtilisateur getFrameAfficherUtilisateur() {
		return frameAfficherUtilisateur;
	}

	public void setFrameAfficherUtilisateur(FrameAfficherUtilisateur frameAfficherUtilisateur) {
		this.frameAfficherUtilisateur = frameAfficherUtilisateur;
	}
